...
make_plot<-function(forecasts,mod_name){

  forecasts |> dplyr::ungroup() |>
    dplyr::filter(model_name==mod_name|Age==min(Age),
           ReturnYear>=min(ReturnYear[!is.na(Pred)])) |>
    dplyr::mutate(age2=ifelse(Age==max(Age),"Total adult",paste("Age",Age))
           ) |>
    ...
    ggplot2::geom_linerange(ggplot2::aes(ymin=L90,ymax=U90),col="firebrick4",alpha=.75)+
    ggplot2::geom_linerange(ggplot2::aes(ymin=L50,ymax=U50),col="firebrick4",lwd=1.25,alpha=.75)+
