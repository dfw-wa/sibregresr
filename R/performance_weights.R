...
#'
#' @details
#' AICc weights \eqn{w_m} for model \eqn{m} are calculated as:
#'  \deqn{w_m=\frac{e^{-0.5\left(\mathrm{AIC}_m-\mathrm{AIC}_{\min }\right)}}{\sum_{i \in \mathcal{M}} e^{-0.5\left(\mathrm{AIC}_i-\mathrm{AIC}_{\min }\right)}}}
#'
#' MAPE and RMSE weights are calculated as, for example:
#'  \deqn{w_m=\frac{\mathrm{MAPE}_m^{-1}}{\sum_{i \in \mathcal{M}} \mathrm{MAPE}_i^{-1}}}
#'
